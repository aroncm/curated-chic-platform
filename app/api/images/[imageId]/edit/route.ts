...
export async function POST(
  ...
  try {
    console.log(`Processing image with Vercel AI Gateway (Imagen): ${originalImageUrl}`);
    ...
    console.log('Calling Vercel AI Gateway (Imagen 4.0)...');

    // Configure Google provider for Vercel AI Gateway
    const googleProvider = google('imagen-4.0-fast-generate-001', {
      apiKey: process.env.GOOGLE_AI_API_KEY,
    });

    // Generate edited image with Imagen via Vercel AI Gateway
    ...
    const editedBuffer = Buffer.from(await generatedImage.arrayBuffer());
